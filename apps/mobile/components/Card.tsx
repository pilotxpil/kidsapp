...
export function PointsBadge({ points, size = 'sm' }: PointsBadgeProps) {
  const { colors, borderRadius, pointsEmoji, heroGradient, id: themeId } = useTheme();
  const styles = useMemo(
    () =>
      StyleSheet.create({
        wrap: {
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        xpText: { color: colors.textMuted, fontSize: 12 },
        barBg: {
          height: 14,
  ...
  return (
    <View style={styles.levelContainer}>
      <View style={[styles.levelHeader, rtl.rowBetween]}>
        <Text style={[styles.levelText, rtl.text]}>⚔️ רמה {level}</Text>
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        streakText: { color: colors.text, fontWeight: '800', fontSize: 16 },
      }),
    [themeId, colors, borderRadius, cardBorder]
  );
