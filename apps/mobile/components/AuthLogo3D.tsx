...
import { createExpoThreeRenderer } from '../lib/threeGl';
...
/** Expo Go’s GL context is incomplete for Three — skip 3D there. */
...
export function AuthLogo3D({ variant = 'gem', size = 120, style }: Props) {
  ...
  const [failed, setFailed] = useState(!shouldUse3D());
  ...
  const onContextCreate = (gl: ExpoWebGLRenderingContext) => {
    try {
      ...
      const renderer = createExpoThreeRenderer(gl);
      ...
      console.warn('[AuthLogo3D] WebGL unavailable', err instanceof Error ? err.message : err);
      setFailed(true);
    }
  };
  ...
  if (failed) {
    return (
      <View style={[{ width: size, height: size, alignItems: 'center', justifyContent: 'center' }, style]}>
        <Text style={{ fontSize: size * 0.55 }}>{FALLBACK[variant]}</Text>
      </View>
    );
  }
